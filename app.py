...
def main():
    ...
    if st.session_state.get('mostrar_config', False):
        with st.expander("⚙️ CALIBRACIÓN MANUAL DE POSICIONES", expanded=True):
            st.info(
                "💡 Ajusta los sliders y ve en tiempo real dónde quedará la hora. La posición seleccionada aparece en **AZUL**, las demás en rojo.")
            ...
            if not any(r.get('hora') for r in st.session_state.rotulos):
                ...
                with col_controles:
                    ...
                    posicion_sel = st.selectbox(
                        ...
                        range(1, 13),
                        format_func=lambda
                            x: f"R{x:02d} - {'Izquierda' if x % 2 == 1 else 'Derecha'} (Fila {(x - 1) // 2 + 1})",
                        key="cal_pos_select"
                    )
